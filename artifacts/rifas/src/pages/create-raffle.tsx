...
export default function CreateRaffle() {
  ...
  const onSubmit = (data: FormValues) => {
    ...
    const payload = {
      ...
      prizes:
        data.type === "multiple_prizes"
          ? prizesList.filter((p) => p.trim() !== "")
          : [],
      singlePrizeAmount:
        data.type === "single_amount" ? data.singlePrizeAmount : undefined,
    };
      ...
      <Card className="border-t-4 border-t-primary shadow-2xl">
        <CardHeader className="text-center pb-8 pt-10">
          ...
          <CardDescription className="text-base">
            Configura los detalles de tu próximo sorteo (Automáticamente se
            crearán 100 números del 0 al 99).
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        className="pl-8"
                        placeholder="1000"
                        {...register("pricePerNumber")}
                      />
                    </div>
                    {errors.pricePerNumber && (
                      <p className="text-sm text-destructive font-medium">
                        {errors.pricePerNumber.message}
                      </p>
                    )}
                  </div>
            ...
            <div className="space-y-6 bg-muted/30 p-6 rounded-2xl border border-border/50">
              ...
              <div className="grid grid-cols-2 gap-4">
                <label
                  className={`cursor-pointer rounded-xl border-2 p-4 flex flex-col items-center text-center transition-all ${watchType === "single_amount" ? "border-primary bg-primary/5 shadow-md text-primary" : "border-border hover:border-primary/50"}`}
                >
                  <input
                    type="radio"
                    value="single_amount"
                  ...
                  <span className="text-xs opacity-80">
                    Un ganador recibe un monto en efectivo
                  </span>
                </label>
                <label
                  className={`cursor-pointer rounded-xl border-2 p-4 flex flex-col items-center text-center transition-all ${watchType === "multiple_prizes" ? "border-primary bg-primary/5 shadow-md text-primary" : "border-border hover:border-primary/50"}`}
                >
                  <input
                    type="radio"
                    value="multiple_prizes"
                    className="sr-only"
                    {...register("type")}
                  />
                  <span className="font-bold text-lg mb-1">
                  ...
                  <span className="text-xs opacity-80">
                    Uno o varios objetos, servicios u otros premios
                  </span>
                </label>
              </div>
              ...
              <div className="mt-6">
                {watchType === "single_amount" ? (
                  <div className="space-y-2 animate-in fade-in">
                    <Label htmlFor="singlePrizeAmount">
                      Monto del Premio (CRC)
                    </Label>
                    <div className="relative">
                      <span className="absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground font-medium">
                        ₡
                      ...
                      <Input
                        id="singlePrizeAmount"
                        type="number"
                        step="1"
                        className="pl-8 text-lg font-bold"
                        placeholder="500000"
                        {...register("singlePrizeAmount")}
                      />
                    </div>
                    {errors.singlePrizeAmount && (
                      <p className="text-sm text-destructive font-medium">
                        {errors.singlePrizeAmount.message}
                      </p>
                    )}
                  </div>
                ) : (
                  <div className="space-y-4 animate-in fade-in">
                    ...
                    {prizesList.map((prize, index) => (
                      <div key={index} className="flex gap-2 items-center">
                        <Badge
                          variant="outline"
                          className="w-8 h-8 flex items-center justify-center p-0 rounded-full bg-background shrink-0"
                        >
                          {index + 1}
                        </Badge>
                        <Input
            ...
            >
              {createMutation.isPending ? (
                ...
                <>
                  <Sparkles className="w-5 h-5 mr-2" /> Generar Rifa y 100
                  Números
                </>
              )}
            </Button>
