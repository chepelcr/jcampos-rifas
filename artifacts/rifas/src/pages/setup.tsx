...
import { Palette, ShieldCheck, Smartphone, Ticket } from "lucide-react";
...
export default function Setup({ onDone, compact = false }: { onDone?: () => void; compact?: boolean }) {
  ...
  const submit = (event: React.FormEvent) => {
    ...
    saveSettings({ configured: true, appName: appName.trim() || "Gestor de Rifas", primaryColor, accentColor });
    onDone?.();
  };
    ...
    <div className={compact ? "bg-background" : "min-h-screen grid lg:grid-cols-2 bg-background"}>
      ...
      <main className={compact ? "py-4" : "flex items-center justify-center p-6 sm:p-12"}>
        <form onSubmit={submit} className="w-full max-w-lg space-y-8">
          {!compact && <div className="lg:hidden w-14 h-14 rounded-2xl bg-gradient-to-br from-primary to-accent flex items-center justify-center"><Ticket className="text-white" /></div>}
          <div>
