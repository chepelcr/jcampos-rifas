...
export async function downloadBuyerConfirmation(raffle: Raffle, buyer: Buyer, raffleNumbers: RaffleNumber[]) {
  const assigned = raffleNumbers.filter((item) => item.buyerId === buyer.id).sort((a, b) => a.number - b.number)
  if (!assigned.length) throw new Error("Este comprador no tiene números asignados.")

  const columns = Math.min(5, assigned.length)
  const rows = Math.ceil(assigned.length / columns)
  const footerY = 590 + rows * 175
  const canvas = document.createElement("canvas")
  canvas.width = 1080
  canvas.height = Math.max(1350, footerY + 180)
  ...
  ctx.fillText(raffle.drawDate ? `Sorteo: ${formatRaffleDate(raffle.drawDate, "d 'de' MMMM, yyyy")}` : "Fecha del sorteo por definir", 540, 285)

  ctx.fillStyle = "#ffffff"
  roundedRect(ctx, 60, 380, 960, Math.max(760, rows * 175 + 190), 30)
  ctx.fill()
  ...
  ctx.fillText("TUS NÚMEROS", 540, 450)
  const cellSize = 130
  const gap = 28
  ...
  assigned.forEach((item, index) => {
    ...
    const y = 500 + Math.floor(index / columns) * 175
    ...
    ctx.fillText(String(item.number).padStart(2, "0"), x + cellSize / 2, y + 62)
    ctx.fillStyle = item.paymentStatus === "paid" ? "#15803d" : "#b45309"
    ctx.font = "700 18px system-ui"
    ctx.fillText(item.paymentStatus === "paid" ? "PAGADO" : "PENDIENTE", x + cellSize / 2, y + 103)
  })

  const paidCount = assigned.filter((item) => item.paymentStatus === "paid").length
  const total = assigned.length * raffle.pricePerNumber
  ctx.fillStyle = "#0f172a"
  ctx.font = "700 28px system-ui"
  ctx.fillText(`${assigned.length} ${assigned.length === 1 ? "número" : "números"} · ${formatCurrency(total)}`, 540, footerY)
  ctx.fillStyle = paidCount === assigned.length ? "#15803d" : "#b45309"
  ctx.font = "700 24px system-ui"
  ctx.fillText(paidCount === assigned.length ? "Pago confirmado" : `${paidCount} pagado(s) · ${assigned.length - paidCount} pendiente(s)`, 540, footerY + 45)
  ctx.fillStyle = "#94a3b8"
  ctx.font = "20px system-ui"
  ctx.fillText("Conserva esta imagen como confirmación de tus números.", 540, footerY + 100)
  ...
  link.href = url
  link.download = `confirmacion-${raffle.name}-${buyer.name}.png`.replace(/[^a-zA-Z0-9áéíóúÁÉÍÓÚñÑ._-]+/g, "-")
